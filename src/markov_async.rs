...
impl MarkovAsync{
    ...
    pub fn get_random_init_word(keys : &Arc<Mutex<Vec<std::string::String>>>, mut rng_thread : StdRng) -> String{
        keys.clone().lock().unwrap().choose(&mut rng_thread).unwrap().to_string()
    }
    ...
    pub fn generate_answer(keys: &Arc<Mutex<Vec<String>>>, cloned : &ReadHandle<String, ChainObjects>, income_text: &str) -> Result<String, Box<dyn std::error::Error>>{
        ...
        match EvMapHandlerAsync::get_item(&cloned, &income_text.to_string()){
            ...
            None => {
                let init_word = MarkovAsync::get_random_init_word(&keys, rng_thread);
        ...
        while answer.len() < 300{
            ...
            answer.push_str(MarkovAsync::continue_sentence(&keys, &answer, &cloned_r, rng).as_str());
            answer.push_str(" ");
        }
        ...
        match lword{
            Some(last_word) => {
                match EvMapHandlerAsync::get_item(&cloned_reader, &last_word.to_string()){
                    ...
                    None => {
                        MarkovAsync::get_random_init_word(&keys, rng_thread).to_string()
                    }
                }
            }
            None => {
